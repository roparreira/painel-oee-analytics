...
import { 
    ...
    ResponsiveContainer, ComposedChart, Cell, LabelList, PieChart, Pie, 
    ScatterChart, Scatter, ZAxis, ReferenceLine, ReferenceArea, ReferenceDot 
} from 'recharts';
import { 
    Upload, Activity, Droplet, AlertTriangle, Calendar, Filter, CheckCircle, 
    BarChart2, FileSearch, Database, ArrowRight, Settings, Clock, RefreshCw, 
    X, ChevronDown, Info, Layout, TrendingDown, Timer, Wrench, Layers, 
    Target, Crosshair, PlayCircle, StopCircle, Percent, Maximize, CalendarX 
} from 'lucide-react';
...
const ComparisonCard = ({ title, real, target, unit = "h", inverse = false, showDeviationOnly = false }) => {
...
const CheckCardDual = ({ title, sub, icon: Icon, valNorte, totalNorte, valSul, totalSul, color = COLORS.orange }) => {
            ...
            <div className="space-y-2">
                <div>
                    ...
                    <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{width: `${pctNorte}%`, backgroundColor: color}}></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{width: `${pctSul}%`, backgroundColor: color}}></div>
                    </div>
                </div>
            </div>
        </Card>
...
const CheckCardSingle = ({ title, value, total, sub, icon: Icon, color = COLORS.orange }) => {
...
// --- COMPONENTE JACK KNIFE CHART ---
const CustomJackKnifeChart = ({ data, title, onPointClick, selectedId, type = "equip" }) => {
    if (!data || data.length === 0) return <div className="flex h-full items-center justify-center text-gray-400 text-xs italic">Sem dados suficientes.</div>;
            ...
            <div className="flex-1 min-h-0 relative">
                <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 10 }}>
                        ...
                        <Scatter name="Limit" data={curveRed} line={{ stroke: COLORS.red, strokeWidth: 2, strokeDasharray: '5 5' }} shape={() => null} isAnimationActive={false} legendType="none" />
                        <Scatter name="Target" data={curveBlue} line={{ stroke: COLORS.blue, strokeWidth: 2, strokeDasharray: '5 5' }} shape={() => null} isAnimationActive={false} legendType="none" />
                        ...
                        >
                            {data.map((entry, index) => {
                                ...
                                const isHighFreq = entry.frequency > (maxFreq/4); 
                ...
                <div className="absolute top-2 right-2 flex flex-col items-end gap-1 pointer-events-none">
                    <span className="text-[9px] text-red-500 font-bold bg-white/80 px-1 rounded">Limite Disp.</span>
                    <span className="text-[9px] text-blue-600 font-bold bg-white/80 px-1 rounded">Meta Disp.</span>
                </div>
            </div>
        </Card>
    );
...
const BigNumberCard = ({ title, valueNumeric, displayValue, unit, target, compact = false }) => {
...
// FIX: New helper that correctly calculates strict overlap with 08:00 - 13:00 window
...
export default function App() {
  ...
  const handleProcess = async () => {
    ...
    } catch(e) { setErrorLog(e.message); } finally { setLoading(false); }
  };
  ...
  const { topEquipmentsData, topCausesData } = useMemo(() => {
    ...
    const processPareto = (obj) => Object.entries(obj).map(([name, value]) => ({ name, value: Math.round(value) })).sort((a, b) => b.value - a.value).slice(0, 10);
        ...
        {step === 'dashboard' && (
            <div className="flex items-center justify-between">
                <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
                    <button onClick={() => setActiveTab('overview')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='overview' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Visão Geral</button>
                    <button onClick={() => setActiveTab('tree')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='tree' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Árvore OEE</button>
                    <button onClick={() => setActiveTab('losses')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='losses' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Análise de Perdas</button>
                    <button onClick={() => setActiveTab('reliability')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='reliability' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Confiabilidade</button>
                    <button onClick={() => setActiveTab('breakdown')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='breakdown' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Desdobramento OEE</button>
                    <button onClick={() => setActiveTab('verification')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab==='verification' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Verificação</button>
                </div>
            ...
            <div className="h-full flex flex-col gap-4 animate-fade-in">
                ...
                {(filterSelection || equipmentFilter) && (
                    <div className="bg-orange-50 border border-orange-200 text-orange-800 px-4 py-2 rounded-lg flex justify-between items-center text-xs font-bold shadow-sm">
                        <div className="flex gap-4">
                            {filterSelection && <span className="flex items-center gap-2"><Filter size={14}/> Período: {calculatedData.find(d => d.key === filterSelection)?.label}</span>}
                            {equipmentFilter && <span className="flex items-center gap-2"><Wrench size={14}/> Equipamento: {equipmentFilter}</span>}
                        </div>
                        <div className="flex gap-2">
                            {filterSelection && (
                                <button onClick={() => setFilterSelection(null)} className="flex items-center gap-1 hover:underline text-orange-600">
                                    <X size={14}/> Limpar Período
                                </button>
                            )}
                            {equipmentFilter && (
                                <button onClick={() => setEquipmentFilter(null)} className="flex items-center gap-1 hover:underline text-orange-600">
                        ...
                        <div className="grid grid-cols-6 gap-2 h-full content-start">
                            ...
                            <div className="col-span-2 h-24">
                                <ComparisonCard 
                                    ...
                                    target={activeAggregates.loadingMins * (1 - TARGETS.AVAIL/100) / 60} 
                            ...
                            <div className="col-span-2 h-24">
                                <ComparisonCard 
                                    ...
                                    target={0} 
                                    real={(activeAggregates.extMaintMins + activeAggregates.outsideMaintMins) / 60} 
                                    inverse={true}
                                />
                            </div>
                            ...
                            <div className="col-span-1 h-32">
                                <Card className="p-3 flex flex-col gap-2 border-l-4 h-full justify-between" style={{borderLeftColor: COLORS.orange}}>
                                    <div>
                                        <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">Duração</p>
                                        <div className="flex gap-2">
                                            <div>
                                                <span className="text-lg font-bold text-slate-700 block">{activeAggregates.windowAvgDurNorte}%</span>
                                                <span className="text-[8px] text-slate-400">Norte</span>
                                            </div>
                                            <div className="w-px bg-slate-200"></div>
                                            <div>
                                                <span className="text-lg font-bold text-slate-700 block">{activeAggregates.windowAvgDurSul}%</span>
                            ...
                            <div className="col-span-2 h-24">
                                <ComparisonCard 
                                    ...
                                    target={0} 
                                    real={(activeAggregates.opsLossMins + activeAggregates.extProdMins + activeAggregates.outsideProdMins) / 60} 
                                    inverse={true}
                                />
                            </div>
                        </div>
                    ...
                    <div className="h-full relative overflow-auto pb-4">
                        ...
                        <div className="flex flex-col items-center pt-8 w-full max-w-5xl mx-auto">
                            ...
                            <div className="flex flex-col items-center mb-8 relative z-10 w-full max-w-4xl">
                                <Card className="w-64 p-4 border-t-4 text-center shadow-lg" style={{borderTopColor: COLORS.blue}}>
                                    <h3 className="font-bold text-slate-500 uppercase text-xs mb-1">OEE Global</h3>
                                    <span className={`text-4xl font-bold block ${parseFloat(activeAggregates.oee) >= TARGETS.OEE ? 'text-green-500' : 'text-red-500'}`}>{activeAggregates.oee}%</span>
                                    <div className="mt-2 pt-2 border-t border-slate-100 text-xs text-slate-400 flex justify-between">
                                        <span>Meta: {TARGETS.OEE}%</span>
                                        <span className={`${parseFloat(activeAggregates.oee) >= TARGETS.OEE ? 'text-green-500' : 'text-red-500'}`}>
                            ...
                            <div className="flex justify-between w-full max-w-5xl px-4 mb-8 relative z-10 gap-4">
                                ...
                                <div className="flex flex-col items-center relative">
                                    ...
                                    <Card className="w-64 p-4 border-t-4 text-center shadow-md hover:shadow-lg transition-all mb-4" style={{borderTopColor: COLORS.red}}>
                                        ...
                                        <span className={`text-3xl font-bold block ${parseFloat(activeAggregates.avail) >= TARGETS.AVAIL ? 'text-green-500' : 'text-red-500'}`}>{activeAggregates.avail}%</span>
                                ...
                                <div className="flex flex-col items-center relative">
                                    <div className="h-4 w-px bg-slate-300 absolute -top-8"></div>
                                    <Card className="w-64 p-4 border-t-4 text-center shadow-md hover:shadow-lg transition-all mb-4" style={{borderTopColor: COLORS.yellow}}>
                                        <h3 className="font-bold text-slate-500 uppercase text-xs mb-1 flex justify-center items-center gap-1">
                                            <Clock size={12}/> Performance
                                        </h3>
                                        <span className={`text-3xl font-bold block ${parseFloat(activeAggregates.perf) >= TARGETS.PERF ? 'text-green-500' : 'text-red-500'}`}>{activeAggregates.perf}%</span>
                                ...
                                <div className="flex flex-col items-center relative">
                                    <div className="h-4 w-px bg-slate-300 absolute -top-8"></div>
                                    <Card className="w-64 p-4 border-t-4 text-center shadow-md hover:shadow-lg transition-all mb-4" style={{borderTopColor: COLORS.green}}>
                                        <h3 className="font-bold text-slate-500 uppercase text-xs mb-1 flex justify-center items-center gap-1">
                                            <CheckCircle size={12}/> Qualidade (Yield)
                                        </h3>
                                        <span className={`text-3xl font-bold block ${parseFloat(activeAggregates.qual) >= TARGETS.QUAL ? 'text-green-500' : 'text-red-500'}`}>{activeAggregates.qual}%</span>
                ...
                {activeTab === 'losses' && (
                    <div className="grid grid-cols-3 grid-rows-2 gap-6 h-full pb-4">
                        <Card className="col-span-2 row-span-1 p-4">
                             <h3 className="font-bold text-slate-700 text-sm mb-2">Evolução das Perdas (Horas)</h3>
                             <div className="flex-1 min-h-0 h-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <YAxis yAxisId="left" fontSize={10} tickLine={false} axisLine={false} tick={{fill: '#94A3B8'}} 
                                            label={{ value: 'Acumulado (h)', angle: -90, position: 'insideLeft', style: {textAnchor: 'middle', fill: '#94A3B8', fontSize: 10} }} />
                                        
                                        <YAxis yAxisId="right" orientation="right" fontSize={10} tickLine={false} axisLine={false} tick={{fill: '#94A3B8'}}
                                            label={{ value: 'Pontual (h)', angle: 90, position: 'insideRight', style: {textAnchor: 'middle', fill: '#94A3B8', fontSize: 10} }} />
                                        ...
                                        <Bar yAxisId="right" dataKey="lossDisp" name="Disp. (Pontual)" fill={COLORS.blue} fillOpacity={0.6} barSize={20} radius={[4, 4, 0, 0]}>
                                            {calculatedData.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fillOpacity={filterSelection && entry.key !== filterSelection ? 0.2 : 0.6} />
                                            ))}
                                        </Bar>
                                        <Bar yAxisId="right" dataKey="lossUtil" name="Perf. (Pontual)" fill={COLORS.yellow} fillOpacity={0.6} barSize={20} radius={[4, 4, 0, 0]}>
                                            {calculatedData.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fillOpacity={filterSelection && entry.key !== filterSelection ? 0.2 : 0.6} />
                                            ))}
                                        </Bar>

                                        <Line yAxisId="left" type="monotone" dataKey="accLossDisp" name="Disp. (Acum.)" stroke={COLORS.blue} strokeWidth={2} dot={false} opacity={filterSelection ? 0.3 : 1} />
                                        <Line yAxisId="left" type="monotone" dataKey="accLossUtil" name="Perf. (Acum.)" stroke={COLORS.yellow} strokeWidth={2} dot={false} opacity={filterSelection ? 0.3 : 1} />
                                    </ComposedChart>
                                </ResponsiveContainer>
                             </div>
                        </Card>
                        ...
                        >
                            ...
                            <h3 className="font-bold text-gray-500 uppercase text-xs mb-4 flex items-center gap-2">
                                {lossFilter === 'availability' && <CheckCircle size={14} className="text-blue-600"/>} Perdas de Disponibilidade
                            </h3>
                            <div className="flex-1 flex flex-col justify-center gap-4">
                                <div>
                                    <span className="text-3xl font-bold block" style={{color: COLORS.blue}}>{activeAggregates.lossDispH} h</span>
                                    <span className="text-xs text-gray-400">Total Indisponível {equipmentFilter ? `(${equipmentFilter})` : ''}</span>
                                </div>
                                <div className="h-px bg-slate-100 w-full"></div>
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div>
                                        <span className="block font-bold text-slate-700">{activeAggregates.lossFailH} h</span>
                                        <span className="text-gray-400">Falhas Téc.</span>
                                    </div>
                                    <div>
                                        <span className="block font-bold text-slate-700">{(parseFloat(activeAggregates.lossDispH) - parseFloat(activeAggregates.lossFailH)).toFixed(1)} h</span>
                        ...
                        >
                            ...
                            <h3 className="font-bold text-gray-500 uppercase text-xs mb-4 flex items-center gap-2">
                                {lossFilter === 'performance' && <CheckCircle size={14} className="text-yellow-600"/>} Perdas de Performance
                            </h3>
                            <div className="flex-1 flex flex-col justify-center gap-4">
                                <div>
                                    <span className="text-3xl font-bold block" style={{color: COLORS.yellow}}>{activeAggregates.lossUtilH} h</span>
                                    <span className="text-xs text-gray-400">Total Perda Ritmo/Ops {equipmentFilter ? `(${equipmentFilter})` : ''}</span>
                                </div>
                                <div className="h-px bg-slate-100 w-full"></div>
                                <div className="text-xs text-gray-400 leading-relaxed">
                                    Calculado base: Capacidade Teórica - Realizado. Inclui microparadas, redução de velocidade e trocas de turno.
                                </div>
                            </div>
                            <div className="absolute bottom-2 right-2 text-[10px] text-gray-300 italic">Clique para filtrar tipo</div>
                        </Card>
                        ...
                        <div className="col-span-2 row-span-1">
                            <Card className="h-full p-4 flex flex-row items-center justify-between bg-slate-50 border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-600">Eventos de Ruído Filtrados</h4>
                                        <p className="text-xs text-slate-400">Eventos únicos com baixo MTTR removidos dos gráficos para clareza (Princípio Pareto 80/20)</p>
                                    </div>
                                </div>
                                <div className="flex gap-8 px-8 border-l border-slate-200">
                                    <div>
                                        <span className="block text-2xl font-bold text-slate-700">{jackKnifeData.noise.length}</span>
                                        <span className="text-[10px] text-slate-400 uppercase font-bold">Eventos</span>
                                    </div>
                                    <div>
                                        <span className="block text-2xl font-bold text-slate-700">{(jackKnifeData.noise.reduce((a,b)=>a+b.totalDuration,0)/60).toFixed(1)} h</span>
