...
const UploadScreen = ({ onDataReady }) => {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            // Passa o resultado completo (dados + auditoria + logs) para o componente pai
            onDataReady(result);
        } catch(e) { 
            console.error(e);
    ...
    return (
        <div className="h-full flex items-center justify-center animate-fade-in">
            <Card className="p-10 max-w-2xl w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="relative border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:bg-slate-50 hover:border-orange-400 transition-all group cursor-pointer">
                        <input 
                            ...
                            accept=".xlsx,.csv" 
                            onChange={e => setFiles(prev => ({...prev, stop: e.target.files[0]}))}
                        />
                    ...
                    <div className="relative border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:bg-slate-50 hover:border-green-400 transition-all group cursor-pointer">
                        <input 
                            ...
                            className="absolute inset-0 opacity-0 cursor-pointer z-10" 
                            accept=".xlsx,.xlsm" 
                            onChange={e => setFiles(prev => ({...prev, prod: e.target.files[0]}))}
                        />
                ...
                {errorLog && (
                    <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm flex items-start gap-3 animate-fade-in">
