...
const ParetoChart = memo(({ data, color, emptyMessage = "Sem dados", onBarClick, selectedName }) => {
    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 35, left: 5, bottom: 0 }} barCategoryGap={2}>
          ...
          <Tooltip cursor={{fill: 'rgba(0,0,0,0.03)'}} contentStyle={{fontSize: '11px', borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', padding: '8px'}} formatter={(val) => [`${val} min`, 'Duração']} />
          <Bar dataKey="value" barSize={12} radius={[0, 4, 4, 0]} cursor="pointer" onClick={(e) => { if (e && onBarClick) { onBarClick(e.name); } }}>
