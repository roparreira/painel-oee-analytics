...
const renderCustomLabel = (props, color) => {
    ...
    const total = (payload.lossDisp || 0) + (payload.lossUtil || 0);
...
const LossEvolutionChart = memo(({ data, onDrillDown, selectedKey, selectedType }) => {
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="horizontal" stackOffset="expand" margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
                <XAxis dataKey="label" fontSize={10} tickLine={false} axisLine={false} tick={{ fill: '#94A3B8' }} dy={5} />
                <YAxis tickFormatter={(val) => toPercent(val, 0)} fontSize={10} tickLine={false} axisLine={false} tick={{ fill: '#94A3B8' }} />
                <Tooltip cursor={{ fill: 'rgba(0,0,0,0.05)' }} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontSize: '12px' }} formatter={(value, name, props) => {
                ...
                <Bar dataKey="lossDisp" name="Perda Disp." stackId="a" fill={COLORS.blue} barSize={40} onClick={(data) => onDrillDown && onDrillDown(data.key, 'availability')} cursor="pointer">
                    {data.map((entry, index) => (<Cell key={`cell-disp-${index}`} fillOpacity={getOpacity(entry.key, 'availability', selectedKey, selectedType)} />))}
                    <LabelList content={(props) => renderCustomLabel(props, '#FFFFFF')} />
                </Bar>
                <Bar dataKey="lossUtil" name="Perda Perf." stackId="a" fill={COLORS.yellow} barSize={40} onClick={(data) => onDrillDown && onDrillDown(data.key, 'performance')} cursor="pointer">
                    {data.map((entry, index) => (<Cell key={`cell-perf-${index}`} fillOpacity={getOpacity(entry.key, 'performance', selectedKey, selectedType)} />))}
